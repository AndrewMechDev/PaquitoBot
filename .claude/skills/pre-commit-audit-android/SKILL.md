...
| 2026-08-19 | `ChatScreen` (root propio, fuera de `AppTabScaffold` - arquitectura correcta) tenía OTRO problema distinto: el saludo completo ("Hola, {nombre}" + subtítulo, `DisplayLarge` 34sp) vivía FIJO fuera del `verticalScroll`, junto con los chips de sugerencia SIEMPRE visibles en el footer - ambos consumían espacio permanente de la vista aunque no fueran doble-scroll. Además no había auto-scroll al último mensaje: el mensaje más reciente quedaba tapado por el footer fijo | Kotlin/UX | Un header/footer fijo NO necesita ser un bug de doble-scroll para "consumir espacio" — cualquier bloque grande fuera del área scrolleable es candidato a mover adentro (ej. saludo pasa a ser el primer item de la lista) si no es indispensable que quede siempre visible (la flecha de "volver" sí lo es, el saludo no). Y toda lista de chat necesita `LaunchedEffect(messages.size) { scrollState.animateScrollTo(scrollState.maxValue) }` — sin esto el scroll no sigue la conversación |
